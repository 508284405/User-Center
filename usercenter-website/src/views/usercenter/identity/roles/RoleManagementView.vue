...
import RoleMenuPermission from '@/components/permission/RoleMenuPermission.vue'
...
// 处理菜单权限设置
function handleMenuPermission(role: Role) {
  currentRoleId.value = role.id
  menuPermissionVisible.value = true
}

// 菜单权限保存后的回调
function handleMenuPermissionSaved() {
...
<template>
  <div class="role-management">
      ...
      <el-table v-loading="loading" :data="roles" class="role-table">
        ...
        <el-table-column label="操作" width="250">
          <template #default="{ row }">
            ...
            <button class="permission-btn" @click="handleMenuPermission(row)">权限设置</button>
    ...
    <RoleMenuPermission
      ...
      @saved="handleMenuPermissionSaved"
    />
  </div>
</template>
